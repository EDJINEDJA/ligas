...
class fbref():
    ...
    def TeamsInfo(self, league: str) -> dict:
        ...
        current_team_stats = {
            row.find_all('td')[0].find("a").get_text(strip=True) if row.find_all('td')[0].find("a") else "": {
                "rank": index + 1,
                "logo": row.find_all('td')[0].find('img')['src'] if row.find_all('td')[0].find('img') else np.nan,
    ...
    def TeamInfos(self,  team: str, league : str) -> dict:
        ...
        stats_categories = {'players' : {'re': 'players', 'header': 1}, 
        ...
        for cat in stats_categories.keys():
            if cat != 'players':
                teamInfos['current stats'][cat] = self._categorystats(soup, stats_categories[cat]['re'] , stats_categories[cat]['header'])
            else:
    ...
    def _categorystats(soup: BeautifulSoup, category: str, header: int) -> pd.DataFrame:
        ...
        table = soup.find('table', {'class': re.compile('stats'), 'id': re.compile(f'{category}')})
